...
class CustomAuthController extends Controller
{
        ...
        if ($user) {
            if ($user->status == 'active') { // Check the user's status
                if (Hash::check($request->password, $user->password)) {
                    ...
                    {
                        return redirect('addattractions')->with('success', 'Login Successful');
                    }else if ($user->usertype=='guider')
                    {
                        return redirect('guiderdashboard')->with('success', 'Login Successful');
                    }
    ...
    public function verifyOTP(Request $request) {
        ...
        if ($user && $user->otp == $request->otp) {
            // Clear OTP and redirect to password reset page
    ...
    public function resetPassword(Request $request) {
        ...
        if ($user) {
            ...
            // Clear the session and redirect the user to the login page with a success message
